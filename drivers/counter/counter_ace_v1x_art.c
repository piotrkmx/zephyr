...
{
	uint32_t val = sys_read32(ACE_TSCTRL);

	val = ((val & ~BIT(ACE_TSCTRL_IONTE_OFFSET))
		| (new_timestamp_enable << ACE_TSCTRL_IONTE_OFFSET));
...
static void counter_ace_v1x_art_cdmas_set(uint32_t cdmas)
{
	...
	val = ((val & ~BIT(ACE_TSCTRL_CDMAS_OFFSET))
		| (cdmas << ACE_TSCTRL_CDMAS_OFFSET));
...
static void counter_ace_v1x_art_ntk_set(bool new_timestamp_taken)
{
	...
	val = ((val & ~BIT(ACE_TSCTRL_NTK_OFFSET))
		| (new_timestamp_taken << ACE_TSCTRL_NTK_OFFSET));
...
{
	return (sys_read32(ACE_TSCTRL) & (1 << ACE_TSCTRL_NTK_OFFSET)) >> ACE_TSCTRL_NTK_OFFSET;
}
...
static void counter_ace_v1x_art_hhtse_set(bool enable)
{
	...
	val = ((val & ~BIT(ACE_TSCTRL_HHTSE_OFFSET))
		| (enable << ACE_TSCTRL_HHTSE_OFFSET));
...
static int counter_ace_v1x_art_get_value(const struct device *dev, uint64_t *value)
{
	ARG_UNUSED(dev);
